...
let lib_paths dir =
  ...
  in
  match
    ( pick [ "libssl.dylib"; "libssl.so" ]
    , pick [ "libcrypto.dylib"; "libcrypto.so" ] )
  with
...
let normalize_libs libs =
  let dirs =
    List.filter_map
      ...
      libs
  in
  match find_map lib_paths dirs with Some libs -> libs | None -> libs
...
let cflags_default c : C.Pkg_config.package_conf =
  ...
  then
    ...
    then { libs = []; cflags = [ "-I/usr/local/opt/openssl/include" ] }
    else { libs = []; cflags = [] }
  else { libs = []; cflags = [] }
